...
<br/>
<br/>
<form method="post" action="{$http_module_path}">
{"Visible_name"|i18n}
...
<a href="{$http_module_path}?s={$s}&go=add_link">{"Add link"|i18n}</a><br/>
...
<tr>
    <td><a href="{$http_module_path}?s={$s}&go=edit_link&id={$link.id}">{"Edit"|i18n}</a></td>
    <td><a href="{$http_module_path}?s={$s}&action=delete_link&id={$link.id}">{"Delete"|i18n}</a></td>
...
{/if}
    ...
    <td>
	{if $link.position > 1}
	    <a href="{$http_module_path}?s={$s}&action=up&id={$link.id}">{"Up"|i18n}</a>
	{/if}
	{if $link.position < $partner_links->get_next_position() - 1}
	    <a href="{$http_module_path}?s={$s}&action=down&id={$link.id}">{"Down"|i18n}</a>
	{/if}
    </td>

    <td>
	<textarea name="url_external">{literal}<script type="text/javascript" src="{/literal}{$http_module_path}dynamic_link.php?id={$link.id}"{literal}></script>{/literal}</textarea>
    </td>
</tr>
{foreachelse}
<tr>
