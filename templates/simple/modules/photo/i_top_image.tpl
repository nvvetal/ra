...
<div style="text-align:center;width:200px;"><a href="{$http_project_path}photo/?go=user_album_photos&s={$s}&album_id={$albumObj->id}&user_id={$albumObj->owner_id}">{$albumObj->name}</a> :: {$photoObj->name|default:$unknown_photo}</div>
</div>