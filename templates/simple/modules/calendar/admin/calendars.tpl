...
<div class="calendar">
    <table width="100%" border="1">
        <tr align="center">
            {foreach from=$Utils->get_months($current_year, $current_month) item=month_data}
                <td {if $current_year == $month_data.year && $current_month == $month_data.month_short}style="background-color:green;"{/if}>
                    <a href="{$http_module_path}index.php?a_id={$smarty.request.a_id}&a_sid={$smarty.request.a_sid}&s={$s}&ago=calendars&current_year={$month_data.year}&current_month={$month_data.month_short}">{$month_data.month|i18n}</a>
                </td>
            {/foreach}
        </tr>
    </table>
