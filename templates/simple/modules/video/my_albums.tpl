...
{foreach from=$userAlbums.items item=albumData}
    ...
    <center>
       ...
       <a href="javascript:void(0);" onclick="showVideoEdit({$albumData->id},'{$s}');"><img src="{$http_images_static_path}icons/edit_24x24.png" alt="" onclick="showVideoAlbumEdit('{$albumData->id}', '{$s}')" /></a>
    </center>
    </div>
{foreachelse}
    {"No user albums"|i18n}
