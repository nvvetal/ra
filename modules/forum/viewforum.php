	...
	$sql = 'SELECT COUNT(topic_id) AS num_topics
		...
		WHERE forum_id = $forum_id
 			AND ((topic_type <> " . POST_GLOBAL . " AND topic_last_post_time >= $min_post_time)
 				OR topic_type = " . POST_ANNOUNCE . ")		" . (($auth->acl_get('m_approve', $forum_id)) ? '' : 'AND topic_approved = 1');
	$result = $db->sql_query($sql);
...
//RAKS_NEW_TOPIC_URL
$newTopic = ($auth->acl_get('f_post', $forum_id) || $user->data['user_id'] == ANONYMOUS) ? append_sid("forum/posting.$phpEx", 'mode=post&f=' . $forum_id, false) : '';
$newTopicURL = RAKS_NEW_TOPIC_URL.'&amp;forum_id='.$forum_id.'&amp;url='.urlencode($newTopic);
