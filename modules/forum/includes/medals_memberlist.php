...
if($result = $db->sql_query($sql))
{
	...
	if ( $medal_count )
	{
		...
		$template->assign_block_vars('switch_display_medal.medal', array(
            'MEDAL_BUTTON' => '<button onclick="hdr_toggle(\'toggle_medal\',\'medal_open_close\')">' . $user->lang['MEDALS_VIEW_BUTTON'] . '</button>'
		));
	}
}
...
for ($i = 0; $i < count($category_rows); $i++)
{
	...
	if ($result = $db->sql_query($sql))
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			{
				$rowset[$row['name']]['medal_issue'] .= $medal_time . $user->format_date($row['time']) . $awarder_name . '</td></tr><tr><td>' . $medal_reason . $reason . '<hr />';
			}
			else
			{
				$rowset[$row['name']]['medal_issue'] = $medal_time . $user->format_date($row['time']) . $awarder_name . '</td></tr><tr><td>' . $medal_reason . $reason . '<hr />';
			...
			if ( !empty($display_medal) )
			{
	            if ($data['medal_count'] > 1)
				{
					...
					else
					{
	                			...
	                			//$data['image'] = substr_replace($data['image'],$cluster, -4) . substr($data['image'], -4) ;
