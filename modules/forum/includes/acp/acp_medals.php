...
class acp_medals
{
	...
	function main($id, $mode)
    {
		...
		$version = new acp_medals_info();
		
		$medals_mod_version = $version->module();
		$medals_mod_version = $medals_mod_version['version'];
		
		if (!isset($config['medals_mod_version']))
		{
			$version->install();
		}
		else if (version_compare($config['medals_mod_version'], $medals_mod_version, '<'))
		{
			$version->update($config['medals_mod_version']);
		}
		...
		switch($submode)
		{
			...
			case 'deletemedal':
				...
				$sql = 'DELETE FROM ' . $table_prefix . 'medals WHERE id = ' . $medal_id;
				$db->sql_query($sql);
				$sql = 'DELETE FROM ' . $table_prefix . 'medals_awarded WHERE medal_id = ' . $medal_id;
				$db->sql_query($sql);
			...
			case 'addcat':
				...
				$sql = 'INSERT INTO ' . $table_prefix . 'medals_cats ' . $db->sql_build_array('INSERT', array(
			...
			case 'addmedalsql':
				...
				$sql = 'INSERT INTO ' . $table_prefix . 'medals ' . $db->sql_build_array('INSERT', array(
					'name'		=> utf8_normalize_nfc(request_var('name', '', true)),
					'image'		=> $_POST['image'],
					'device'	=> utf8_normalize_nfc(request_var('device', '', true)),
					'dynamic'	=> $_POST['dynamic'],
			...
			case 'deletecat':
				...
				{
					$sql = 'DELETE FROM ' . $table_prefix . 'medals_cats WHERE id = ' . $cat_id;
					$db->sql_query($sql);
					$i = 1;
					foreach ($medals as $key => $value)
					{
						if ($value['parent'] == $_POST['newcat'])
						{
							$i = $value['order_id'] + 1;
						}
					}
				...
				else if (isset($_POST['deleteall']))
				{
					$sql = 'DELETE FROM ' . $table_prefix . 'medals_cats WHERE id = ' . $cat_id;
					$db->sql_query($sql);
					$sql = 'DELETE FROM ' . $table_prefix . 'medals WHERE parent = ' . $cat_id;
					$db->sql_query($sql);
					foreach ($medals as $key => $value)
					{
						if ($value['parent'] == $cat_id)
						{
							$sql = 'DELETE FROM ' . $table_prefix . 'medals_awarded WHERE medal_id = ' . $value['id'];
							$db->sql_query($sql);
						}
					}
