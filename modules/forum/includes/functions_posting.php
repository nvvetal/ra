...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true)
{
	...
	require_once('Rating_forum.class.php');
	
	
	$Rating_forum = new Rating_forum(RATING_SERVER_URL);
	
	
	// Submit new topic
	if ($post_mode == 'post')
	{
		//$rating_class->add_rating_value(intval($user->data['user_id']),"forum_thread");
		//$Rating_forum->send_action('add',intval($user->data['user_id']),"forum_thread");
	...
	{
		//$rating_class->add_rating_value(intval($user->data['user_id']),"forum_post");
		$countType = (strtotime('first day of week') >= time() && time() <= strtotime('first day of next week') ? 'firstWeek' : 'nextWeek');
        sendRaksMoney($user->data['user_id'], 'forumMessage', array('countType'=> $countType));
		$Rating_forum->send_action('add',intval($user->data['user_id']),"forum_post");		
		if ($post_mode == 'reply')
		{
			$sql_data[POSTS_TABLE]['sql'] = array_merge($sql_data[POSTS_TABLE]['sql'], array(
				'topic_id' => $data['topic_id'])
