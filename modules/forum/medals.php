...
if ( $mode == "mi" )
{
	...
	$image  = create_from_extention($medal);

	imagecolortransparent($image,imagecolorat($image,0,0));

	if ( file_exists($device) and $device <> '' )
	{
		$insert = create_from_extention($device);
...
switch ($mode)
{
	...
	case 'submit_nomination':
		...
		$db->sql_query($sql);
        sendRaksMoney($user->data['user_id'], 'forumMedal');
		trigger_error(sprintf($user->lang['MEDAL_NOMINATE_GOOD'], append_sid('memberlist.php?mode=viewprofile&u=' . $user_id)));
	break;
	...
	case 'awarded':
		...
		$template->set_filenames(array(
			'body' => 'medalcp_awarded_user.html')
		);
		page_footer();
	break;
	...
	case 'submit':
		...
		trigger_error(sprintf($user->lang['MEDAL_AWARD_GOOD'], append_sid('memberlist.php?mode=viewprofile&u=' . $user_id)));
	break;
	...
	case 'delete':
		...
		$sql = "DELETE FROM " . MEDALS_AWARDED_TABLE . "
				...
				LIMIT 1";
		$db->sql_query($sql);
		trigger_error(sprintf($user->lang['MEDAL_REMOVE_GOOD'], append_sid('medals.php?m=awarded&u=' . $user_id)));
	break;
	...
	case 'approve':
		...
		trigger_error(sprintf($user->lang['MEDAL_AWARD_GOOD'], append_sid('medals.php?m=validate&u=' . $user_id)));
	break;
	...
	case 'validate':
		...
		$template->set_filenames(array(
			'body' => 'medalcp_nominate_user.html')
		);
		page_footer();
...
//			$username = $usernames = explode("\n", $usernames) ;
			...
			{
				trigger_error(sprintf($user->lang['NOT_MEDALS_AWARDED'], implode(", ", $not_award_user), append_sid('medals.php?mode=' . $mode . '&med=' . $med_id))) ;
			}
			else
			{
				trigger_error(sprintf($user->lang['MEDAL_AWARD_GOOD'], append_sid('medals.php?mode=' . $mode . '&med=' . $med_id)));
			}
		}
			...
			foreach ($medals as $key2 => $value2)
			{
				if ($value2['parent'] == $value['id'])
				{
					...
					$template->assign_block_vars('medalrow', array(
							...
							'U_MEDAL_CP'		=> 'medals.php?m=ma&med=' . $value2['id'],
