...
if (!empty($topic_data['poll_start']))
{
    ...
    //if($user->data['user_posts'] < 5) $s_can_vote = false;
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
    ...
    if ($user_cache[$poster_id]['sig'] && $row['enable_sig'] && empty($user_cache[$poster_id]['sig_parsed']))
    {
        ...
        $user_cache[$poster_id]['sig'] = preg_replace('@<a(.*?href=[\"\']http://.*?)>(.*?)</a>@is','<a rel="nofollow"$1>$2</a>', $user_cache[$poster_id]['sig']);
    ...
    $message = preg_replace("/<a ([^>]*)>([^\<]*)\<\/a>/i","<a $1 target='_blank'>$2</a>",$message);

    $message = preg_replace('@<a(.*?href=[\"\']http://.*?)>(.*?)</a>@is','<a rel="nofollow"$1>$2</a>',$message);
    //$user_sig = preg_replace('@<a(.*?href=[\"\']http://.*?)>(.*?)</a>@is','<a rel="nofollow"$1>$2</a>',$user_sig);
    ...
    $editLink = ($edit_allowed) ? append_sid("{$phpbb_root_path}posting.$phpEx", "mode=edit&amp;f=$forum_id&amp;p={$row['post_id']}") : '';
...
//-- end: Prime Warnings ----------------------------------------------------//
    ...
    if ( $config['medal_display_topic'] > 0 && $medals_count > 0)
    {
		...
		FROM " . $table_prefix . "medals m, " . $table_prefix . "medals_awarded ma, " . $table_prefix . "medals_cats c
