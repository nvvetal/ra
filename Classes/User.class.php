...
class User
{
    ...
    function authorize_user_by_login_pass($login, $password)
    {
        ...
        $query = "
            SELECT *
            FROM users
            WHERE login=" . SQLQuote($login) . " OR email = ".SQLQuote($login)."
            LIMIT 1
        ";
    ...
    function is_user_id_exists($user_id)
    {
        $query = "
                ...
                LIMIT 1
            ";
        return !is_null(SQLGet($query, $this->dbh));
    }
     ...
     * @return array|null|string|void
     */
    function find_user_by_facebook_id($facebookId)
    {
        $query = "
            SELECT *
            FROM users
            WHERE facebook_id = " . SQLQuote($facebookId) . "
        ";
        return SQLGet($query, $this->dbh);
    }
    ...
    function facebookAuth($token)
    {
        ...
        $userProfile = $facebook->getMyProfile($token);
     ...
     * @return string
     */
    public function generateLogin($login, $i = 0)
    {
        $username = $i == 0 ? $login : $login . $i;
        $userId = $this->find_user_id_by_login($username);
        if ($userId == 0) return $username;
        $i++;
        return $this->generateLogin($login, $i);
    }
